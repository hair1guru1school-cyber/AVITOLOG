begin;
alter table public.frontend_state_records drop constraint if exists frontend_state_records_storage_key_check;
alter table public.frontend_state_records add constraint frontend_state_records_storage_key_check check (
  storage_key in ('avitolog_clients','avitolog_projects','avito_kp_saved_client_packages_v1','avito_kp_custom')
...
begin
 ...
 if not (
  p_key in ('avitolog_clients','avitolog_projects','avito_kp_saved_client_packages_v1','avito_kp_custom')
 ...
 select organization_id into v_org from public.organization_members where user_id=auth.uid() and role in('owner','admin') order by case role when 'owner' then 0 else 1 end limit 1;
 if v_org is null then raise exception 'Owner or admin role required';end if;
 insert into public.frontend_state_records(organization_id,storage_key,value_text,updated_by) values(v_org,p_key,p_value,auth.uid())
 on conflict(organization_id,storage_key) do update set value_text=excluded.value_text,revision=public.frontend_state_records.revision+1,updated_by=auth.uid()
